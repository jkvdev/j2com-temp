...
$catid = Factory::getApplication()->input->get(
	...
	array(),
	'Array'
);	//	Removed 1 instance (there were 2 exact ones)

// Initialize retrieved category Id
$default_catid = '';
...
<form
	...
	<!-- Hidden Input for category filter -->
	<input
		type="hidden"
		name="filter_catid"
		id="filter_catid"
	...
	if ($this->params->get('list_show_filter_category', 0)):
		...
		if (
			...
			<div class="j2store-product-filters category-filters">
				...
				<h4 class="product-filter-heading">
					<!-- Text -->
					<?= Text::_('J2STORE_PRODUCT_FILTER_CATEGORY_TITTLE'); ?>
				</h4>

				<?php
				// Check if category toggle is enabled
				if (
					$this->params->get('list_filter_category_toggle', 1)
					&& $this->params->get('list_filter_category_toggle', 1) == 1
					||  !empty($filter_catid)
				): ?>
					<!-- Render " - " icon -->
					<a
						href="javascript:void(0);"
						id="cat-filter-icon-minus"
						onclick="getCategoryFilterToggle(this);">
					...
					<a
						href="javascript:void(0);"
						id="cat-filter-icon-plus"
						onclick="getCategoryFilterToggle(this);"
						style="display:none;">
						<i class="icon-plus"></i>
					</a>

					<!-- Display Category DIV as block -->
					<div id="j2store_category" style="display:block;">
					<?php

				// if category filter is set to 2
				elseif (
					$this->params->get('list_filter_category_toggle', 1) == 2
					&& $this->params->get('list_filter_category_toggle', 1)
				): ?>
						<!-- Icon	" + "	-->
						<a
						...
						<a
							href="javascript:void(0);"
							id="cat-filter-icon-plus"
							onclick="getCategoryFilterToggle(this);" style="display:none;">
							<i class="icon-minus"></i>
						</a>

						<!-- Display category div hidden -->
						<div id="j2store_category" style="display:none;">
						<?php else:
						// Default:
						?>
							<!-- Display category div -->
							<div id="j2store_category">
							<?php endif; ?>

							<!-- List of all the filters -->
							<ul
								id="j2store_categories_mod"
								class="nav nav-list nav-stacked j2store-category-list"
								style="display: block !important;">
								<?php
								// Check if all categories should be shown
								if ($this->params->get('list_show_filter_category_all', 0)): ?>
									<!-- Generate list item for all filters -->
									<li class="<?= isset($filter_catid) && $filter_catid == '' ? 'active' : ''; ?> j2product-categories  level1">
										<!-- Link -->
										<a
											href="javascript:void(0);"
											class="j2store-item-rootcategory"
								...
								foreach ($this->filters['filter_categories'] as $item) : ?>
									<!-- Generate list item -->
									<li class="<?= (!empty($filter_catid) && $filter_catid == $item->id) ? 'active' : ''; ?> j2product-categories  level<?= $item->level ?>">
										<?php
										// If the item level is greater than 1
										if ($item->level > 1): ?>
											<!-- Sub Link -->
											<a
												href="javascript:void(0);"
